...
import {Http, Jsonp, URLSearchParams, RequestOptions} from 'angular2/http';
import 'rxjs/add/operator/map';

@Component({
  selector: 'news',
  template: '<div class="news medium">{{item}}</div>'
})
export class News implements OnInit {
  ...
  private feeds = [];
  private newsItems = [];
  private seenNewsItem = [];
  private _yqURL = 'https://query.yahooapis.com/v1/public/yql';
  private params = {
    q: 'select * from rss where url=',
    format: 'json'
  };
  private _failedAttempts = 0;
  private fetchInterval = moment.duration(1, 'hour');
  private updateInterval = moment.duration(6, 'seconds');
  private item;
  ...
  private fetchNews() {
    // Reset the news feed
    this.newsItems = [];
    this.feeds.forEach(_curr => {
      var options = this.buildOptions(_curr);
      this.fetchFeed(options);
    });
  }
   ...
   */
  private fetchFeed(params) {
    // let headers = new Headers({ 'Content-Type': 'application/json' });
    let options = new RequestOptions({ search: params });

    this.jsonp.get(this._yqURL, options)
      .map(res => res.json())
    // .catch(this.handleError)
      ...
      if (data.query.count > 0) {
        ...
        console.error('No feed results for: ', params);
      }
    });
  }
  ...
  private showNews() {
    ...
    var _item = this.newsItems.splice(_location, 1)[0];
  ...
  public ngOnInit() {
    ...
    this.fetchInterval = this.config.fetchInterval || this.fetchInterval;
